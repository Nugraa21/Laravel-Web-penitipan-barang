        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 px-4 lg:px-0">
            <div>
                <h1 class="text-3xl font-black text-gray-900 tracking-tight uppercase flex items-center gap-3">
                    <span
                        class="w-10 h-10 rounded-xl bg-blue-600 text-white flex items-center justify-center shadow-lg shadow-blue-500/30">
        ...
        <div class="glass-card shadow-xl overflow-hidden rounded-2xl border border-white/60 mx-4 lg:mx-0">
            @if($conversations->count() > 0)
                <div class="divide-y divide-gray-100/50" id="chat-list">
                    @foreach($conversations as $conv)
                        <a href="{{ route('chat.index', $conv->user->id) }}"
                            class="flex items-center gap-4 p-5 hover:bg-white/60 transition-colors {{ $conv->unread_count > 0 ? 'bg-blue-50/30' : '' }} group relative">
                            <!-- Avatar -->
                            <div
                                class="relative shrink-0 w-14 h-14 rounded-full bg-gradient-to-br from-gray-100 to-gray-200 border-2 border-white shadow-sm flex items-center justify-center text-gray-700 font-black text-xl group-hover:scale-105 transition-transform">
                                {{ strtoupper(substr($conv->user->name, 0, 1)) }}
                                @if($conv->unread_count > 0)
                                    <span
                                        class="absolute top-0 right-0 w-3.5 h-3.5 bg-blue-500 border-2 border-white rounded-full"></span>
                                @endif
                            </div>
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center justify-between mb-1">
                                    ...
                                    @if($conv->last_message)
                                        <span
                                            class="text-xs font-bold {{ $conv->unread_count > 0 ? 'text-blue-600' : 'text-gray-400' }} shrink-0">
                                            {{ $conv->last_message->created_at->isToday() ? $conv->last_message->created_at->format('H:i') : $conv->last_message->created_at->format('d M') }}
                                        </span>
                                    @endif
                                </div>
                                <div class="flex items-center justify-between gap-4">
                                    ...
                                    @if($conv->unread_count > 0)
                                        <span
                                            class="shrink-0 w-5 h-5 rounded-full bg-blue-600 text-white text-[0.65rem] font-bold flex items-center justify-center shadow-md shadow-blue-500/30">
                                            {{ $conv->unread_count }}
                                        </span>
                                    @endif
                                </div>
            ...
            @else
                <div class="p-12 text-center text-gray-500 flex flex-col items-center justify-center">
                    <div
                        class="w-20 h-20 rounded-full bg-gray-100 border-4 border-white flex items-center justify-center mb-4 shadow-sm">
